...
class Game:
    def __init__(game_self, SCREEN_WIDTH, SCREEN_HEIGHT, OFFSET, game_clock): #automatically executed when an instance of that class is created
        ...
        game_self.run = True
        game_self.game_over_img = pygame.image.load("Graphic/game_over.png").convert_alpha()
        game_self.game_over_img = pygame.transform.scale(game_self.game_over_img, (400,400))
        game_self.restart_button = Button((SCREEN_WIDTH - 120)/2 + 20, SCREEN_HEIGHT - 250) #create restart button with pos x, y for rect
        ...
        game_self.explosion_sound = pygame.mixer.Sound("Sound/explosion.ogg")
        game_self.hit_sound = pygame.mixer.Sound("Sound/hit.wav")
        game_self.mshit_sound = pygame.mixer.Sound("Sound/ms.wav")
